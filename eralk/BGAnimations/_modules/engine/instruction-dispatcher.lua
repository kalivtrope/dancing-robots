...
local function check_if_all_notes_processed()
  if processed_number_of_batches >= total_number_of_batches then
    ...
    for pn=1,max_pn do
      ...
      if player then
        if distinguish_player_score then
          GAMESTATE:Env()[Constants.played_num_of_steps][pn] = total_numbers_of_hits[pn]
        else
          GAMESTATE:Env()[Constants.played_num_of_steps][default_pn] = (GAMESTATE:Env()[Constants.played_num_of_steps][default_pn] or 0) + total_numbers_of_hits[pn]
        end
      end
    end
  end
...
local function inc_hits(pn)
  ...
  total_numbers_of_hits[pn] = (total_numbers_of_hits[pn] or 0) + 1
end
    ...
    if player then
      ...
      if distinguish_player_score then
        total_numbers_of_notes[pn] = count_notes_from_note_data(player:GetNoteData())
          GAMESTATE:Env()[Constants.total_num_of_steps][pn] = total_numbers_of_notes[pn]
        ...
        total_numbers_of_notes[default_pn] =
          ...
          GAMESTATE:Env()[Constants.total_num_of_steps][default_pn] = total_numbers_of_notes[default_pn]
      end
    end
  end
